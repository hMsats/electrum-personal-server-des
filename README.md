...
Note that if the Trezor, Electrum and EPS use zpubs (BIP84 bc1... addresses), the descriptor wallet needs xpubs. Therefore the zpub in Electrum (Wallet -> Information) needs to be converted to a xpub using [xpub-converter](https://jlopp.github.io/xpub-converter) (which also gives the "fingerprint") and read into the full node descriptor wallet like this: 
